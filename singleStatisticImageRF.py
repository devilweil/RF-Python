...
def readSingleImageMultiVIBySample(samples):
...
def predictImage(clt,reftif):
    os.chdir(r'H:\gansu\wuwei\S2-Statisitic')
    dirList = sorted(glob.glob('NDVI*.tif'))
    indice=len(dirList)
    ###循环加载同一个指数的遥感影像
    imgDataSet=[]
    for i in range(indice):
...
if __name__ == "__main__": 
    ...
    x,y=readSingleImageMultiVIBySample(X)
...
#    non_lable_yOrigin = non_lable_transform[3]
    ...
    #导出到TIF中
    savePath=r'H:\gansu\wuwei\预测结果\VI\NDVI-predict080-singleimage.tif'
